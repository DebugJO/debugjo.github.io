...
데이터베이스는 기본적으로 'select'와 같이 조회했을 때 그 결과가 운반 단위(array)에 도달하면 호출한 쪽으로 그 결과를 바로 리턴해준다.  예를 들어 어떤 테이블에 row 개수에 1000만 개가 있든 10만 개가 있든지 상관없이 `select * from tablename`을 실행하면 바로 화면에 나와야 정상이다.
