...
* 전체 : [LINQ-Lambda Tutorial](https://github.com/DebugJO/HelloWorldSample/blob/master/CSharp/LinqTutorial.cs)

```cs
public static string GetDisPlayName<T>(string fieldName) where T : new()
{
    var nameInfo = typeof(T).GetProperty(fieldName);
    return nameInfo == null ? fieldName : nameInfo.GetCustomAttributes(typeof(DisplayNameAttribute), true).Cast<DisplayNameAttribute>().Single().DisplayName;
}
```
