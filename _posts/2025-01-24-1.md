...
* 참고로 위에서 직접 작성한 소스를 [**deepseek**](https://www.deepseek.com/)에 질문했더니 아래와 같은 답변을 해주었다. 비교해서 학습하는 데 도움이 되길 바라면서 소스코드를 아래에 옮겨본다. 옮기는 과정에서 약간의 소스 정리는 하였다.
...
```cs
public sealed class DisposeList<T> : IDisposable  
{  
